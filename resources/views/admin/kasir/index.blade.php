...
@section('topbar-title', 'Point of Sale Terminal')
...
<div class="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:min-h-[calc(100vh-180px)]" x-data="kasir()">
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 gap-4 items-start content-start">
                    ...
                    <div class="product-item" x-show="keyword === '' || '{{ addslashes(strtolower($product->nama_produk)) }}'.includes(keyword.toLowerCase())">
                        <div class="bg-white border border-slate-200 hover:border-blue-500 hover:bg-slate-50 rounded-xl p-3 cursor-pointer transition-all duration-200 flex flex-col"
                             @click="addToCart({{ $product->id }}, '{{ addslashes($product->nama_produk) }}', {{ $product->harga }}, {{ $product->stok }})">
                            ...
                            <div class="mt-2 flex justify-between items-center">
                                <span class="font-bold text-slate-900 text-lg">Rp {{ number_format($product->harga, 0, ',', '.') }}</span>
                                <span class="px-2 py-1 bg-slate-100 text-slate-500 rounded text-xs" x-text="'Stok: ' + getSisaStok({{ $product->id }}, {{ $product->stok }})">Stok: {{ $product->stok }}</span>
                            </div>
                        </div>
                    </div>
                @endforeach
    ...
    <div class="lg:col-span-5 h-full">
        <form action="{{ route('admin.kasir.process') }}" method="POST" class="flex flex-col gap-6 h-full">
            @csrf
