    ...
    <div class="bg-white rounded-xl shadow-md p-6 md:p-10 text-center">
        ...
        <div class="bg-gray-50 border border-gray-100 rounded-xl p-5 md:p-6 text-left mb-8">
            <div class="flex justify-between items-center mb-3">
                ...
                <strong class="text-sm font-bold text-gray-900">{{ $order->kode_transaksi }}</strong>
            </div>
            <div class="flex justify-between items-center mb-4">
                <span class="text-sm font-medium text-gray-500">Tanggal</span>
                <strong class="text-sm font-semibold text-gray-900">{{ $order->created_at->format('d M Y, H:i') }}</strong>
            </div>
            ...
            <div class="space-y-3 mb-4">
                @foreach($order->items as $item)
                    <div class="flex justify-between items-center">
                        <span class="text-sm text-gray-700 font-medium">{{ $item->nama_produk }} <span class="text-gray-400">× {{ $item->jumlah }}</span></span>
        ...
        <a href="{{ route('pembeli.products') }}" class="inline-flex justify-center items-center bg-blue-600 text-white hover:bg-blue-700 font-semibold py-3 px-8 rounded-lg shadow-sm transition-colors w-full sm:w-auto gap-2">
