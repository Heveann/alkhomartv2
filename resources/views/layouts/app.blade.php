...
<head>
    ...
    <title>@yield('title', 'AlkhoMart') — Discover Your Style</title>
...
<body class="font-sans-clean antialiased text-gray-900 bg-white selection:bg-black selection:text-white">
    ...
    <nav x-data="{ mobileMenuOpen: false, scrolled: false }" 
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-20">
                ...
                <div class="flex-shrink-0 flex items-center">
                    <a href="{{ route('pembeli.products') }}" class="font-serif-elegant text-2xl font-bold tracking-tighter">
                        AlkhoMart.
                    </a>
                </div>
                ...
                <div class="hidden lg:flex space-x-10">
                    <a href="{{ route('pembeli.products') }}" class="text-sm font-medium hover:text-gray-500 transition-colors {{ request()->routeIs('pembeli.products') ? 'text-black border-b border-black pb-0.5' : 'text-gray-500 hover:border-b hover:border-gray-500 pb-0.5' }}">Shop</a>
                    <a href="{{ route('pembeli.collection') }}" class="text-sm font-medium hover:text-gray-500 transition-colors {{ request()->routeIs('pembeli.collection') ? 'text-black border-b border-black pb-0.5' : 'text-gray-500 hover:border-b hover:border-gray-500 pb-0.5' }}">Collection</a>
                    <a href="{{ route('pembeli.about') }}" class="text-sm font-medium hover:text-gray-500 transition-colors {{ request()->routeIs('pembeli.about') ? 'text-black border-b border-black pb-0.5' : 'text-gray-500 hover:border-b hover:border-gray-500 pb-0.5' }}">About</a>
                    @auth
                        @if(auth()->user()->isAdmin())
                            <a href="{{ route('admin.dashboard') }}" class="text-sm font-medium text-gray-500 hover:text-black transition-colors">Dashboard Admin</a>
                        @endif
                    @endauth
                </div>
                ...
                <div class="hidden lg:flex items-center space-x-6">
                    @auth
                        @if(auth()->user()->isPembeli())
                            <a href="{{ route('pembeli.cart') }}" aria-label="Keranjang Belanja" class="relative text-black hover:text-gray-500 transition p-1 focus:outline-none focus-visible:ring-2 focus-visible:ring-black rounded">
                        ...
                        <div x-data="{ open: false }" class="relative">
                            <button @click="open = !open" @click.away="open = false" aria-haspopup="true" :aria-expanded="open" class="flex items-center gap-2 text-sm font-medium text-black hover:text-gray-500 transition focus:outline-none focus-visible:ring-2 focus-visible:ring-black rounded p-1">
                                ...
                                <span class="max-w-[100px] truncate hidden xl:inline">{{ explode(' ', auth()->user()->name)[0] }}</span>
                            </button>
                    ...
                    @else
                        @if(!request()->routeIs('login') && !request()->routeIs('register'))
                            <a href="{{ route('login') }}" class="text-sm font-medium text-black hover:text-gray-500 transition">Masuk</a>
                            <a href="{{ route('register') }}" class="text-sm font-medium text-black border border-black px-4 py-2 hover:bg-black hover:text-white transition">Daftar</a>
                        @endif
                    @endauth
                </div>
                ...
                <div class="lg:hidden flex items-center space-x-4">
                    @auth
                        @if(auth()->user()->isPembeli())
                            <a href="{{ route('pembeli.cart') }}" class="text-black relative">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15.75 10.5V6a3.75 3.75 0 10-7.5 0v4.5m11.356-1.993l1.263 12c.07.665-.45 1.243-1.119 1.243H4.25a1.125 1.125 0 01-1.12-1.243l1.264-12A1.125 1.125 0 015.513 7.5h12.974c.576 0 1.059.435 1.119 1.007zM8.625 10.5a.375.375 0 11-.75 0 .375.375 0 01.75 0zm7.5 0a.375.375 0 11-.75 0 .375.375 0 01.75 0z"></path></svg>
                                @php $cartCount = \App\Models\Cart::where('user_id', auth()->id())->sum('jumlah'); @endphp
                                @if($cartCount > 0)
                                    <span class="absolute -top-1.5 -right-1.5 flex items-center justify-center min-w-[18px] h-[18px] text-[10px] font-bold text-white bg-black rounded-full">{{ $cartCount }}</span>
                                @endif
                            </a>
                        @endif
                    @endauth
        ...
        <div x-show="mobileMenuOpen" x-collapse x-cloak class="lg:hidden bg-white border-b border-gray-100 shadow-sm absolute top-full left-0 w-full">
            <div class="px-4 py-6 space-y-4">
                <a href="{{ route('pembeli.products') }}" class="block px-3 py-2 text-lg font-medium {{ request()->routeIs('pembeli.products') ? 'text-black' : 'text-gray-500' }}">Shop</a>
                <a href="{{ route('pembeli.collection') }}" class="block px-3 py-2 text-lg font-medium {{ request()->routeIs('pembeli.collection') ? 'text-black' : 'text-gray-500' }}">Collection</a>
                <a href="{{ route('pembeli.about') }}" class="block px-3 py-2 text-lg font-medium {{ request()->routeIs('pembeli.about') ? 'text-black' : 'text-gray-500' }}">About</a>
                @auth
                    @if(auth()->user()->isAdmin())
                        <a href="{{ route('admin.dashboard') }}" class="block px-3 py-2 text-lg font-medium text-gray-500">Dashboard Admin</a>
                    @endif
                    <div class="border-t border-gray-100 pt-6 mt-4">
                        <div class="px-3 text-sm font-medium text-gray-400 mb-2">Akun: {{ auth()->user()->name }}</div>
                        <form action="{{ route('logout') }}" method="POST">
                            @csrf
                ...
                @else
                    @if(!request()->routeIs('login') && !request()->routeIs('register'))
                        <div class="flex flex-col gap-4 mt-6 px-3">
                            <a href="{{ route('login') }}" class="w-full text-center py-3 border border-black font-medium text-black">Masuk</a>
                            <a href="{{ route('register') }}" class="w-full text-center py-3 bg-black text-white font-medium">Daftar</a>
                        </div>
                    @endif
                @endauth
            </div>
    ...
    <footer class="bg-white pt-24 pb-12 border-t border-gray-100 mt-20">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8 mb-16">
                <!-- Brand -->
                <div>
                    <a href="{{ route('pembeli.products') }}" class="font-serif-elegant text-3xl font-bold tracking-tighter mb-6 block text-black">
                        AlkhoMart.
                    </a>
                ...
                <div>
                    ...
                    <ul class="space-y-4">
                        <li><a href="{{ route('pembeli.products') }}" class="text-gray-500 hover:text-black text-sm transition-colors">All Products</a></li>
    ...
    <script>
        $(document).ready(function() {
            ...
            @if(session('success'))
                Toast.fire({ icon: 'success', title: '{{ session('success') }}', background: '#10b981' });
            @endif

            @if(session('error'))
                Toast.fire({ icon: 'error', title: '{{ session('error') }}', background: '#ef4444' });
            @endif
        });
    </script>
    @stack('scripts')
